...
export function PartnersSectionEn() {
  return (
    <section className="py-8 sm:py-12 md:py-16 bg-white">
      <div className="container mx-auto px-4">
        <div className="text-center max-w-2xl mx-auto mb-8 sm:mb-12">
          <h2 className="text-2xl sm:text-3xl font-bold mb-2 sm:mb-4">Our Partners</h2>
          <p className="text-neutral-600">Leading ocean carriers worldwide</p>
        </div>
          ...
          {partners.map((partner) => (
            <div key={partner.name} className="w-full h-16 sm:h-20 md:h-24 relative flex items-center justify-center group px-2 sm:px-4">
              <div className="relative w-full h-full transition-transform duration-300 group-hover:scale-110">
                <Image src={partner.logo} alt={`${partner.name} Logo`} fill className="object-contain filter grayscale hover:grayscale-0 transition-all duration-300" sizes="(max-width: 640px) 120px, (max-width: 768px) 160px, 200px" />
              </div>
            </div>
          ))}
        </div>
