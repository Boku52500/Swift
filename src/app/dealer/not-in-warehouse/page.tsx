...
export default function NotInWarehousePage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await getCars(CarStatus.IN_TRANSIT)
  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    setError("")
    try {
      const res = await getCars(CarStatus.IN_TRANSIT, undefined, {
        name: name || undefined,
        vin: vin || undefined,
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold">საწყობში არ მისული</h1>
