...
export default function ArrivedCarsPage() {
  ...
  useEffect(() => {
    const fetchCars = async () => {
      try {
        const qs = new URLSearchParams({ status: "ARRIVED" })
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold">ჩამოსული მანქანები</h1>
          ...
          <TableBody>
            {cars.map((car) => (
              <TableRow key={car.id}>
                ...
                <TableCell>{(car as any).auction || "-"}</TableCell>
                <TableCell>{car.buyer || "-"}</TableCell>
                <TableCell>{car.receiver || "-"}</TableCell>
                <TableCell>{(car as any).location || "-"}</TableCell>
