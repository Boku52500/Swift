...
export default function NotInWarehousePage() {
  ...
  useEffect(() => {
    const fetchCars = async () => {
      try {
        const response = await fetch("/api/admin/cars?status=IN_TRANSIT")
  ...
  const fetchWithFilters = async (params?: { name?: string; vin?: string; lotNumber?: string }) => {
    const qs = new URLSearchParams({ status: "IN_TRANSIT" })
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold">საწყობში არ მისული მანქანები</h1>
